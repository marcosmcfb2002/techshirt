...
import { getAllProductCategories, getAllProducts } from '../lib/models/Product';
import dynamic from 'next/dynamic';
import { Button } from '@nextui-org/react';

export const getServerSideProps = async (context: GetServerSidePropsContext) => {
    const categorys = await getAllProductCategories();
    ...
    async function checkNumbers(numbers: number[]) {
        ...
        for (let i = 0; i < numbers.length; i++) {
            if (numbers[i] > 0) {
                if ((await getAllProducts(numbers[i] - 1)).length > 0) {
    ...
    if (page) {
        ...
        products = await getAllProducts(Number(page) - 1, category, filter, priceRange, text)
