...
async function saveResetPasswordToken(_id: object, token: string) {
  ...
  const result = await db
    .collection("users")
    .updateOne({ _id: _id }, { $set: { resetPasswordToken: token } });
  return result;
}
...
async function removeFromWishlist(email: string, productId: string) {
  const result = await db
    .collection("users")
    .updateOne({ email: email }, { $pull: { wishList: productId } as any });
  return result;
}
